...
namespace PayPal.Api
{
  ...
  public class WebhookEvent : PayPalRelationalObject
  {
    ...
    public static WebhookEventList List(APIContext apiContext, int pageSize = 10, string startTime = "",
      string endTime = "")
    {
      // Validate the arguments to be used in the request
      ArgumentValidator.ValidateAndSetupAPIContext(apiContext);
    ...
    public static string ConvertAuthAlgorithmHeaderToHashAlgorithmName(string authAlgorithmHeader)
    {
      ...
      throw new AlgorithmNotSupportedException(string.Format("Unable to map {0} to a known hash algorithm.",
        authAlgorithmHeader));
    }
      ...
      string webhookId = "")
    {
      ...
      try
      {
        ...
        var expectedSignature =
          string.Format("{0}|{1}|{2}|{3}", transmissionId, transmissionTimestamp, webhookId, crc32);
        ...
        var trustedX509Certificate =
          CertificateManager.Instance.GetTrustedCertificateFromFile(apiContext == null ? null : apiContext.Config);

        // Validate the certificate chain.
        isValid = CertificateManager.Instance.ValidateCertificateChain(trustedX509Certificate,
          x509CertificateCollection);

        // Verify the received signature matches the expected signature.
        if (isValid)
        {
          var certificate = x509CertificateCollection[0];
          var rsa = certificate.PublicKey.Key as RSA;
          var signatureBytes = Convert.FromBase64String(signature);
          using (var rsaProvider = new RSACryptoServiceProvider())
          {
            rsaProvider.ImportParameters(rsa.ExportParameters(false));
            isValid = rsaProvider.VerifyData(expectedSignatureBytes, CryptoConfig.MapNameToOID(hashAlgorithm),
              signatureBytes);
          }
        }
      }
